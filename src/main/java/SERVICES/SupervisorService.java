...
import MODELS.DTO.REQUEST.SupervisorRequestDTO;
...
public class SupervisorService {
    ...
    public Page<SupervisorResponseDTO> findSupervisors(Pageable pageable) {
        try {
            return repository.findAll(pageable)
                    .map(SupervisorResponseDTO::convert);
        } catch (Exception e) {
    ...
    public SupervisorResponseDTO findByEmail(String email) {
        Optional<Supervisor> optionalSupervisor = repository.findByEmail(email);
        if (optionalSupervisor.isPresent()) {
            return optionalSupervisor.get().convert();
        } else {
            throw new NaoEncontradoException("Supervisor não encontrado");
        }
    }
