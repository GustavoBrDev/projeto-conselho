...
import conselho.estudante.com.projetoconselho.SERVICES.LOGS.RepresentativeLogsService;
...
/**
 * Controller para gerenciamento de logs de Representatives
 * @author Gustavo Stinghen
 * @since 07/04/2025
 * @see RepresentativeLogsService
 */
@AllArgsConstructor
@RestController
@RequestMapping("/logs/Representatives")
@Tag(name = "Logs de representantes", description = "Recurso de gerenciamento de logs de representantes")
public class RepresentativeLogsController {

    private RepresentativeLogsService service;

    @Operation(summary = "Busca todos os logs")
    @ApiResponse(responseCode = "200", description = "Logs encontrados com sucesso",
            content = @Content(schema = @Schema(implementation = RepresentativeLogs.class),
                    examples = @ExampleObject(value = "{\"id\": \"string\", \"actor\": {\"id\": 1, \"name\": \"Gustavo Stinghen\", \"registration\": 123456, \"email\": \"7G9Gt@example.com\", \"password\": \"123456\", \"image\": \"https://example.com/image.jpg\"}, \"target\": {\"id\": 1, \"name\": \"Representante\"}, \"type\": \"CREATE\", \"timestamp\": \"2023-03-17T14:30:00.000Z\", \"createdAt\": \"2023-03-17T14:30:00.000Z\", \"changes\": [{\"field\": \"nome\", \"oldValue\": \"Representante\", \"newValue\": \"Representante Atualizado\"}]}")))
    @ApiResponse(responseCode = "400", description = "Erro ao buscar logs")
    @ApiResponse(responseCode = "500", description = "Erro interno do servidor")
    @GetMapping()
    public ResponseEntity<Page<RepresentativeLogs>> findAll(Pageable pageable) {
    ...
    @ApiResponse(responseCode = "500", description = "Erro interno do servidor")
    @GetMapping("/actor")
    public ResponseEntity<Page<RepresentativeLogs>> findByActor(
    ...
    public ResponseEntity<Page<RepresentativeLogs>> findByType(
    ...
    @ApiResponse(responseCode = "500", description = "Erro interno do servidor")
    @GetMapping("/target")
    public ResponseEntity<Page<RepresentativeLogs>> findByTarget(
            @RequestBody @Parameter(description = "Alvo do log", required = true, content = @Content(schema = @Schema(implementation = Representative.class)), example = "{\"id\": 1, \"name\": \"Representante\"}") Representative target,
            Pageable pageable) {
